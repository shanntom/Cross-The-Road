...
// Objects defined inherit from the class Shape
// All Shapes have certain arrays.  These each manage either
...
// Based off of UCLA's Graphics Example Code, by Garett for CS174a.


//SELF DEFINED: CHICKBODY
Declare_Any_Class( "ChickBody",    
  { 'populate'()                
      { 
        //right  : 0-9              
        this.positions     .push( vec3(-.75,.5,1),  vec3(-1,.75,1), vec3(-1,0,1), vec3(-.75,-.5,1), vec3(.25,-.5,1), vec3(.5,.25,1) , vec3(1,.25, 1), vec3(1,1,1), vec3(.25,1,1),vec3(.25,.5,1));
        this.normals     .push( vec3(0,0,1),    vec3(0,0,1),  vec3(0,0,1),  vec3(0,0,1),  vec3(0,0,1),    vec3(0,0,1),    vec3(0,0,1), vec3(0,0,1), vec3(0,0,1), vec3(0,0,1));
        this.texture_coords.push( vec2(.125,.75),   vec2(0,.875), vec2(0,.5), vec2(.125,.25), vec2(.625, .25), vec2(.75,.625),  vec2(1,.625),   vec2(1,1),    vec2(.625,1), vec2(.625,.75)  );
        this.indices     .push( 0,1,2,  0,2,3,  0,3,4,  0,4,5,      0,5,9,  9,5,8,  8,5,7,  7,5,6);
        //left  : 10-19
        this.positions     .push( vec3(-.75,.5,-1), vec3(-1,.75,-1),vec3(-1,0,-1), vec3(-.75,-.5,-1), vec3(.25,-.5,-1), vec3(.5,.25,-1) , vec3(1,.25, -1), vec3(1,1,-1), vec3(.25,1,-1),vec3(.25,.5,-1));
        this.normals     .push( vec3(0,0,-1),   vec3(0,0,-1), vec3(0,0,-1), vec3(0,0,-1), vec3(0,0,-1),    vec3(0,0,-1) , vec3(0,0,-1)  , vec3(0,0,-1)  , vec3(0,0,-1)  , vec3(0,0,-1));
        this.texture_coords.push( vec2(.125,.75),   vec2(0,.875), vec2(0,.5), vec2(.125,.25), vec2(.625, .25),  vec2(.75,.625),   vec2(1,.625),   vec2(1,1),    vec2(.625,1), vec2(.625,.75));
        this.indices     .push( 10,11,12, 10,12,13, 10,13,14,   10,14,15,     10,15,19,  19,15,18,  18,15,17,  17,15,16);
        //topside: 20- 23 
        this.positions     .push( vec3(-.75,.5,1),  vec3(-.75,.5,-1), vec3(.25,.5,1), vec3(.25,.5,-1));
    ...
    this.indices     .push( 23,20,22,  21,20,23);
    ...
    var n = 0;
    for (n; n < 9;  n++){
      this.positions.push ( this.positions[n],this.positions[n+1], this.positions[n+b],  this.positions[n+b+1]);
      var v1 = subtract (this.positions[n+1], this.positions[n]);
      var v2 = subtract (this.positions[n+b], this.positions[n] );
      var norm = normalize(cross(v2, v1));
      this.normals.push( norm, norm, norm, norm);
      this.texture_coords.push( vec2(1,1),   vec2(1,0),   vec2(0,1),   vec2(0,0)   ); 
      this.indices.push( curr, curr+1, curr+3,  curr, curr+3, curr+2);
      curr += 4;
    }
      }
...
// UCLA's Graphics Example Code, by Garett for CS174a.
// Contents:
